...
This workflow enables the Designer to save and restore rich session state, including context, artifact links, and future steps.
...
1.  **Deep Capture**: Analyze the current session to extract:
    ...
    -   `current_topic`: What was being worked on (e.g., "Design System Tokens").
    -   `session_summary`: A 1-2 sentence overview of achievements.
    -   `artifacts`: Scan `artifacts/{project_id}/designer/` for any files created/modified in this session.
    -   `key_decisions`: List explicit user choices (e.g., "Primary: #HEX", "Radius: 8px").
...
3.  **Construct YAML**:
    ...
    artifacts:
      - type: "Design Artifact"
        path: "artifacts/{project_id}/designer/{filename}.md"
        description: "Visual Spec for X"
    ```
4.  **Write**: 
    - For Persona: Save to `artifacts/{project_id}/designer/memory/{persona_filename}`.
    - For Workflow: Save to `artifacts/{project_id}/designer/memory/{workflow-id}/{workflow_filename}`.
...
**Steps**:
1.  **List**: Scan `artifacts/{project_id}/designer/memory/` (and subfolders) for available YAML files.
    -   `[1] persona-2026-01-09-01.yaml` (Topic: UI Design)
